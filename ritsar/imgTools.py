...
def DSBP(phs, platform, img_plane, center=None, size=None, derate=1.05, taylor=20, n=32, beta=4, cutoff='nyq',
         factor_max=6, factor_min=0):
    ##############################################################################
    #                                                                            #
    ...
    deltaF = freq[freq.size // 2] - freq[
        freq.size // 2 - 1]  # Assume sample spacing can be determined by difference between last two values (first two are distorted by decimation filter)
    ...
    RPP = sph[1:, 0] - sph[:-1, 0]
    abs_RPP = abs(RPP)
    I = np.argsort(abs_RPP);
    sort_RPP = abs_RPP[I]
    im = I[4];
    RPPdata = sort_RPP[4]  # len(I)/2
...
def DS(phs, platform, img_plane, center=None, size=None, derate=1.05, taylor=20, n=32, beta=4, cutoff='nyq',
       factor_max=6, factor_min=0):
    ##############################################################################
    #                                                                            #
            ...
            for k in range(2):
                for l in range(2):
                    ...
                    tmp = DS(phsDS, platformDS, img_plane_sub,
                             derate=derate, taylor=17, n=n, beta=beta, cutoff=cutoff, factor_max=factor_max,
...
def FFBPmp(phs, platform, img_plane, N=3, derate=1.05, taylor=20, n=32, beta=4, cutoff='nyq', factor_max=2,
           factor_min=0):
    ##############################################################################
    #                                                                            #
